...
export const Digits = () => {
  ...
  for (let i = 0; i < 60; i++) letters[i] = (sec + i) % 60;
...
export const Digits1 = () => {
  ...
  useEffect(() => {
    setInterval(() => {
      sethour(date.getHours() + 1);
    });
  });
  for (let i = 0; i < 12; i++) letters[i] = (hour + i) % 12;
        ...
        {letters.map((ch, i) => {
          return (
            <span
              ...
              className=" absolute origintl1 font-bold text-xl"
            >
              {ch}
            </span>
          );
...
export const Date1 = () => {
  ...
  return (
    <div className="text-5xl">
      {dateInside.day}:{dateInside.month}:{dateInside.year}
    </div>
  );
};
